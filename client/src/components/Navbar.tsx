...
export default function Navbar() {
  ...
  const handleNavigation = (item: { label: string; section?: string; path?: string }) => {
    if (item.path) {
      ...
      <div className="ml-10 flex items-baseline space-x-8">
        {navItems.map((item, index) => (
          <motion.button
            ...
            className={`relative hover:text-brand-teal transition-colors font-medium group ${(item.path && location === item.path) || (!item.path && location === '/' && false) // approximate active state
              ? 'text-brand-teal'
              : 'text-foreground'
              }`}
      ...
      >
        {navItems.map((item, index) => (
          <motion.button
            key={item.label}
            onClick={() => handleNavigation(item)}
            className={`block px-3 py-2 text-foreground hover:text-brand-teal hover:bg-brand-teal/10 transition-all duration-300 font-medium w-full text-left rounded-lg ${(item.path && location === item.path) ? 'bg-brand-teal/5 text-brand-teal' : ''
              }`}
            data-testid={`link-${item.label}-mobile`}
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
